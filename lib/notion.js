...
/**
 * Gets the Notion client and database ID for a specific type.
 * @param {string} envKey - The environment variable key for the database ID.
 * @returns {Promise<{notion: Client, databaseId: string} | null>}
 */
async function getNotionConfig(envKey) {
  ...
  try {
    ...
    if (ctx && ctx.env) {
      ...
      databaseId = ctx.env[envKey];
    }
  } catch (e) {}

  apiKey = apiKey || process.env.NOTION_API_KEY;
  databaseId = databaseId || process.env[envKey];
