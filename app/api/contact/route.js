...
export async function POST(request) {
  try {
    ...
    const recipientEmail = process.env.CONTACT_EMAIL_RECIPIENT || 'dogs_cat@naver.com';
    ...
    console.log('--- ACTUAL SMTP SCRIPT STARTING ---');
...
본 메일은 멍냥의민족 홈페이지 컨택트 섹션에서 발송되었습니다.
    ...
    console.log('--- ACTUAL SMTP SUCCESS ---');
    console.log('MessageId:', info.messageId);

    return NextResponse.json({ message: 'Success' }, { status: 200 });
  } catch (error) {
    console.error('--- ACTUAL SMTP ERROR ---');
